...
#    
# 2. 在本地运行，远程部署：
#    git clone https://github.com/JunyuZhan/pis-cloud.git && cd pis-cloud
...
GITHUB_REPO="${GITHUB_REPO:-https://github.com/JunyuZhan/pis-cloud.git}"
...
deploy_remote() {
    ...
    # 在远程执行
    ssh -t ${SSH_USER}@${SSH_HOST} "${ENV_VARS} curl -sSL https://raw.githubusercontent.com/JunyuZhan/pis-cloud/main/scripts/deploy.sh | bash"
}
...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    ...
    echo ""
    echo "在服务器上运行:"
    echo "  curl -sSL https://raw.githubusercontent.com/JunyuZhan/pis-cloud/main/scripts/deploy.sh | bash"
    echo ""
    echo "在本地远程部署:"
    echo "  bash scripts/deploy.sh <服务器IP> [用户名]"
    echo ""
    echo "非交互式部署（通过环境变量）:"
    echo "  SUPABASE_URL=xxx SUPABASE_SERVICE_ROLE_KEY=xxx \\"
    echo "    curl -sSL https://raw.githubusercontent.com/JunyuZhan/pis-cloud/main/scripts/deploy.sh | bash"
    echo ""
    exit 0
fi

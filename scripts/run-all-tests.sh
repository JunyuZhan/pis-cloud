...
run_test_suite() {
    ...
    if [ ! -f "$script_path" ]; then
        echo -e "${RED}错误: 测试脚本不存在: $script_path${NC}"
    ...
    # 提取测试结果（兼容 macOS grep）
    local passed=$(echo "$output" | grep "通过:" | sed -E 's/.*通过: ([0-9]+).*/\1/' || echo "0")
    local failed=$(echo "$output" | grep "失败:" | sed -E 's/.*失败: ([0-9]+).*/\1/' || echo "0")
    local skipped=$(echo "$output" | grep "跳过:" | sed -E 's/.*跳过: ([0-9]+).*/\1/' || echo "0")
    ...
    if [ $exit_code -eq 0 ]; then
        ...
        echo "$output"
        echo ""
        echo -e "${GREEN}✓ 测试套件 '$suite_name' 完成${NC}"
    else
        TEST_SUITES+=("$suite_name|FAIL: Exit code $exit_code")
        echo "$output"
        echo ""
        echo -e "${RED}✗ 测试套件 '$suite_name' 失败 (退出码: $exit_code)${NC}"
    fi
...
EOF
    ...
    for suite_entry in "${TEST_SUITES[@]}"; do
        ...
        if echo "$status" | grep -q "^PASS:"; then
            echo "| $suite | ✅ PASS | $status |" >> "$REPORT_FILE"
        elif echo "$status" | grep -q "^FAIL:"; then
            echo "| $suite | ❌ FAIL | $status |" >> "$REPORT_FILE"
        else
            echo "| $suite | ⚠️ ERROR | $status |" >> "$REPORT_FILE"
        fi
    done
...
- ❌ **失败**: $TOTAL_FAILED
- ⏭️ **跳过**: $TOTAL_SKIPPED
- 📊 **通过率**: $(if [ $TOTAL_TESTS -gt 0 ]; then echo "scale=1; $TOTAL_PASSED * 100 / $TOTAL_TESTS" | bc; else echo "0"; fi)%
...
print_final_summary() {
    ...
    if [ $TOTAL_FAILED -eq 0 ]; then
        ...
        echo ""
        echo -e "${CYAN}测试报告已保存到: $REPORT_FILE${NC}"
        exit 0
    else
...
main() {
    ...
    # 运行所有测试套件
    run_test_suite "基础功能测试" "$SCRIPT_DIR/test-system.sh"
    run_test_suite "功能测试" "$SCRIPT_DIR/test-functional.sh"
    run_test_suite "API功能测试" "$SCRIPT_DIR/test-api.sh"
    run_test_suite "端到端测试" "$SCRIPT_DIR/test-e2e.sh"
    run_test_suite "高级功能测试" "$SCRIPT_DIR/test-advanced.sh"
    
    # 如果有登录凭据，运行认证测试
    if [ -n "$PIS_ADMIN_EMAIL" ] && [ -n "$PIS_ADMIN_PASSWORD" ]; then
        run_test_suite "认证功能测试" "$SCRIPT_DIR/test-authenticated.sh"
    else
        echo -e "${YELLOW}跳过认证测试: 未提供登录凭据${NC}"
        echo ""
    fi

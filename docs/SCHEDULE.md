...
| 里程碑 | 日期 | 交付物 | 验收标准 |
|--------|------|--------|----------|
| **M0** | Day 0 | 项目启动 | 需求确认、团队就位 |
| **M1** | Day 1 | 环境就绪 | Supabase/MinIO 可连接，代码仓库初始化 |
| **M2** | Day 3 | 后台可用 | 管理员可登录、创建/编辑相册 |
| **M3** | Day 5 | 上传可用 | 照片上传后自动处理，生成缩略图/预览图 |
| **M4** | Day 7 | 访客可用 | 访客可浏览相册、查看大图、下载原图 |
| **M5** | Day 8 | 实时可用 | 上传照片后访客端实时显示 |
| **M6** | Day 10 | 正式上线 | 全链路测试通过，部署生产环境 |
...
### Phase 6: 部署上线 (Day 8-10) 🔵 文档已备

> **Claude 已完成**: Docker Compose、Nginx 配置、DEPLOYMENT.md 部署指南
> **待用户执行**: Vercel 部署、域名配置、服务启动

| ID | 任务 | 负责 | 状态 | 前置 | 工时 | 产出物 |
|----|------|------|------|------|------|--------|
| **P6-01** | 创建 Vercel 项目 | **User** | ⬜ | P0-03 | 0.5h | 见 DEPLOYMENT.md |
| **P6-02** | 配置 Vercel 环境变量 | **User** | ⬜ | P6-01 | 0.5h | 见 DEPLOYMENT.md |
| **P6-03** | 配置自定义域名 | **User** | ⬜ | P6-01 | 0.5h | 见 DEPLOYMENT.md |
| **P6-04** | 部署 Preview 分支 | **User** | ⬜ | P6-02 | 0.5h | Vercel 自动 |
...
| **P6-11** | 部署 Production | **User** | ⬜ | P6-06 | 0.5h | Vercel Deploy |
